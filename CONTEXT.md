...
**Merge strategy:** Conflicts are resolved by a `case` statement in the workflow — fork-owned files keep ours, everything else (blocklists, Python modules, etc.) takes theirs. `git config merge.ours.driver true` is still set but the explicit case statement is the primary protection mechanism.

**Files protected from upstream overwrites:**

- `apply-exclusions.sh`, `my-exclusions.txt`, `my-inclusions.txt`, `my-additions.txt`, `CLAUDE.md`, `CONTEXT.md` — explicitly kept ours in the case statement
- `.github/workflows/sync-upstream.yml` — kept ours via `.github/workflows/*` glob in the case statement; all other upstream workflow files are deleted before commit
- `install.html`, `little-snitch-install.html` — take upstream on merge, then immediately overwritten by Stage 1
- `README.md` — takes upstream on merge, then URLs fixed by Stage 2
